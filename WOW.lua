...
if game.PlaceId == 8304191830 then
	...
	repeat
		task.wait()
	until game.Players.LocalPlayer.PlayerGui:FindFirstChild("collection"):FindFirstChild("grid"):FindFirstChild("List"):FindFirstChild("Outer"):FindFirstChild("UnitFrames")
	repeat
		task.wait()
	until game.ReplicatedStorage.packages:FindFirstChild("assets")
	repeat
		task.wait()
	until game.ReplicatedStorage.packages:FindFirstChild("StarterGui")
else
	repeat
		task.wait()
	until game.Workspace:FindFirstChild(game.Players.LocalPlayer.Name)
	...
	repeat
		task.wait()
	until game:GetService("Workspace")["_waves_started"].Value == true
end
...
if game.PlaceId ~= 8304191830 then GLD() end
...
function get_inventory_items_unique_items() return v19["session"]["inventory"]["inventory_profile_data"]["unique_items"] end
function get_inventory_items() return v19["session"]["inventory"]["inventory_profile_data"]["normal_items"] end
function get_Units_Owner() return v19["session"]["collection"]["collection_profile_data"]["owned_units"] end
...
local function GetCurrentLevelId()
	if game.Workspace._MAP_CONFIG then return game:GetService("Workspace")._MAP_CONFIG.GetLevelData:InvokeServer()["id"] end
end
local function GetCurrentLevelName()
	if game.Workspace._MAP_CONFIG then return game:GetService("Workspace")._MAP_CONFIG.GetLevelData:InvokeServer()["name"] end
end
...
function webhook()
	if Settings.WebhookEnabled then
		...
		for i, v in pairs(get_inventory_items_unique_items()) do
			if string.find(v["item_id"], "portal") or string.find(v["item_id"], "disc") then Table_All_Items_New_data[v["item_id"]]["Count"] = Table_All_Items_New_data[v["item_id"]]["Count"] + 1 end
		end
		for i, v in pairs(get_Units_Owner()) do
		...
		if TextDropLabel == "" then TextDropLabel = "Not Have Items Drops" end
			...
			["embeds"] = {
				{
					...
					["fields"] = {
						...
						{
							["name"] = "เพชรที่ต้องฟาร์ม :",
							["value"] = "```ini\n" .. Settings.GemFramed .. " / " .. Settings.GemtoFarm .. "```",
							["inline"] = false,
						},
					},
				},
...
function SnipeShopNew()
	if Settings.snipeWebhookEnabled then pcall(function()
...
function ShopSniperWebhook()
	if Settings.snipeWebhookEnabled then
		...
		if not game:GetService("Workspace")["travelling_merchant"]["is_open"].Value then table.insert(data["embeds"][1]["fields"], {
			["name"] = "SHOP CLOSED",
			["value"] = "SHOP CLOSED",
			["inline"] = true,
		}) end
...
local function UnitSec()
	...
	function LoadUnits()
		...
		table.clear(Units)
		for i, v in pairs(getgenv().profile_data.equipped_units) do
			if DataUnits[v.unit_id] then table.insert(Units, DataUnits[v.unit_id].name .. " #" .. tostring(v.uuid)) end
		end
		Check()
	end
	function GetUnits()
...
function AutoSaveUnit()
	if Settings.AutoSaveUnit then
		...
		local function fetchUnit()
			...
			for i, v in pairs(getgenv().profile_data.equipped_units) do
				if v.equipped_slot then EquippedList["UP" .. v.equipped_slot] = tostring(v.unit_id) .. " #" .. tostring(v.uuid) end
			end

			-- If Equipped Slot in Empty, Put "nil"
			for i = 1, 6, 1 do
				if EquippedList["UP" .. i] == nil then EquippedList["UP" .. i] = "nil" end
			end

			-- Compared Current List to Saved JSON List if not the same then call AutoSave Function
			for i = 1, 6, 1 do
				if EquippedList["UP" .. i] ~= Settings.SelectedUnits["UP" .. i] then saveUnit() end
			end
		end
...
local function WorldSec()
	...
	getgenv().updateworld = function()
		selectworld:ClearDrop()
		local storylist
		if Settings.WorldCategory == "Story Worlds" then
			storylist = { "Planet Namak", "Shiganshinu District", "Snowy Town", "Hidden Sand Village", "Marine's Ford", "Ghoul City", "Hollow World", "Ant Kingdom", "Magic Town", "Cursed Academy", "Clover Kingdom", "Cape Canaveral", "Alien Spaceship", "Fabled Kingdom", "Hero City", "Puppet Island", "Virtual Dungeon", "Windhym" }
		...
		elseif Settings.WorldCategory == "Raid Worlds" then
			storylist = { "Storm Hideout", "West City", "Infinity Train", "Shiganshinu District - Raid", "Hiddel Sand Village - Raid", "Freezo's Invasion", "Entertainment District", "Hero City (Hero Slayer)", "Marine's Ford (Buddha)" }
	...
	getgenv().updatelevel = function()
		selectlevel:ClearDrop()
		local levellist
		local level = Settings.SelectedWorld
		---///Story Worlds\\\---
	...
	getgenv().updatedifficulty = function()
		selectdiff:ClearDrop()
		level = Settings.SelectedLevel
		cata = Settings.WorldCategory
		local diff
		if level == "namek_infinite" or level == "aot_infinite" or level == "demonslayer_infinite" or level == "naruto_infinite" or level == "marineford_infinite" or level == "tokyoghoul_infinite" or level == "hueco_infinite" or level == "hxhant_infinite" or level == "magnolia_infinite" or level == "jjk_infinite" or level == "clover_infinite" or level == "jojo_infinite" or level == "opm_infinite" or level == "7ds_infinite" or level == "mha_infinite" or level == "sao_infinite" or level == "berserk_infinite" or level == "dressrosa_infinite" or cata == "Legend Stages" or cata == "Raid Worlds" then
			diff = { "Hard" }
	...
	local selectlevel = SelectWorld:Cheat("Dropdown", "🎚️ Din't use Challenge", function(value)
		...
		saveSettings()
	end, { options = { "double_cost", "short_range", "fast_enemies", "regen_enemies", "tank_enemies", "shield_enemies", "triple_cost", "hyper_regen_enemies", "hyper_shield_enemies", "godspeed_enemies", "flying_enemies", "mini_range" }, default = Settings.SelectedChallenge })
	...
	local selectlevel = SelectWorld:Cheat("Dropdown", "🎚️ Din't use Challenge ", function(value)
		...
		saveSettings()
	end, { options = { "double_cost", "short_range", "fast_enemies", "regen_enemies", "tank_enemies", "shield_enemies", "triple_cost", "hyper_regen_enemies", "hyper_shield_enemies", "godspeed_enemies", "flying_enemies", "mini_range" }, default = Settings.SelectedChallenge2 })
	...
	local selectlevel = SelectWorld:Cheat("Dropdown", "🎚️ Din't use Challenge ", function(value)
		...
		saveSettings()
	end, { options = { "double_cost", "short_range", "fast_enemies", "regen_enemies", "tank_enemies", "shield_enemies", "triple_cost", "hyper_regen_enemies", "hyper_shield_enemies", "godspeed_enemies", "flying_enemies", "mini_range" }, default = Settings.SelectedChallenge3 })
...
function refarmcon()
	...
	--findPicoro
	if Settings.picoHOP then Settings.picoHOP = false end
	--Start
	if Settings.autostart then Settings.autostart = false end
	--Place unit
	if Settings.AutoFarm then Settings.AutoFarm = false end
	--Replay
	if Settings.AutoReplay then Settings.AutoReplay = false end
	--Next Story
	if Settings.AutoNext then Settings.AutoNext = false end
	--Leave
	if Settings.AutoLeave then Settings.AutoLeave = false end
	--Abilities
	if Settings.AutoAbilities then Settings.AutoAbilities = false end
	--Upgrade
	if Settings.AutoUpgrade then Settings.AutoUpgrade = false end
	--AutoSell
	if Settings.AutoSell then Settings.AutoSell = false end
	--autoQuitWave
	if Settings.autoQuit then Settings.autoQuit = false end
	--AutoSellWave
	if Settings.AutoSellWave then Settings.AutoSellWave = 0 end
	--Inf Castle--
	--Next Level inf cas
	if Settings.AutoContinue then Settings.AutoContinue = false end
	--Auto Infinity Castle
	if Settings.AutoInfinityCastle then Settings.AutoInfinityCastle = false end
	saveSettings()
	autoload2()
end
...
function setfarm1()
	...
	--Start
	if Settings.autostart then Settings.autostart = true end
	--Place unit
	if Settings.AutoFarm then Settings.AutoFarm = true end
	--Replay
	if Settings.AutoReplay then Settings.AutoReplay = false end
	--Next Story
	if Settings.AutoNext then Settings.AutoNext = false end
	--Leave
	if Settings.AutoLeave then Settings.AutoLeave = true end
	--Abilities
	if Settings.AutoAbilities then Settings.AutoAbilities = true end
	--Upgrade
	if Settings.AutoUpgrade then Settings.AutoUpgrade = true end
	--AutoSell
	if Settings.AutoSell then Settings.AutoSell = true end
	--autoQuitWave
	if Settings.autoQuit then Settings.autoQuit = false end
	--AutoSellWave
	if Settings.AutoSellWave then Settings.AutoSellWave = 50 end
	--Inf Castle--
	--Next Level inf cas
	if Settings.AutoContinue then Settings.AutoContinue = false end
	--Auto Infinity Castle
...
function setfarm2()
	...
	if Settings.AutoFarm then Settings.AutoFarm = true end
	--Replay
	if Settings.AutoReplay then Settings.AutoReplay = true end
	--Next Story
	if Settings.AutoNext then Settings.AutoNext = false end
	--Leave
	if Settings.AutoLeave then Settings.AutoLeave = true end
...
function setfarmIC()
	...
	--Inf Castle--
	--Next Level inf cas
	if Settings.AutoContinue then Settings.AutoContinue = true end
	--Auto Infinity Castle
	if Settings.AutoInfinityCastle then Settings.AutoInfinityCastle = true end
	saveSettings()
	autoload2()
end
...
local function DELMAPNEW()
	...
	DELMAP:Cheat("Button", "Activate Delete Hill ", function() DelHill() end)
end
						...
						for i, v in pairs(Unit:getChildren()) do
							if v:WaitForChild("_stats"):FindFirstChild("base") then
								if tostring(v._stats.base.Value) == "pve" then
									...
									if tonumber(distance) == distanceTable[1] then enemy = v.HumanoidRootPart.CFrame * CFrame.new(0, 0, -4) end
								end
							end
						end
					end
...
local function credits()
	...
	Developers:Cheat("Button", "🔗 Discord Invite", function() setclipboard("https://discord.gg/2ttfCfzxut") end)
...
function saveposTEST(UnitPos, a, a2, a3, a4, a5, a6)
	local GetLevelData = game.workspace._MAP_CONFIG:WaitForChild("GetLevelData"):InvokeServer()
	world = GetLevelData.world or GetLevelData.name
	if game.workspace._MAP_CONFIG:WaitForChild("GetLevelData") then updatepos(world, UnitPos, a, a2, a3, a4, a5, a6) end
	warn("savepos test")
end
--fixmap
...
local function UnitPosSec()
	UnitPosition:Cheat("Button", " Unit 1 Position", function() MouseClick2("UP1") end)
	UnitPosition:Cheat("Button", " Unit 2 Position", function() MouseClick2("UP2") end)
	UnitPosition:Cheat("Button", " Unit 3 Position", function() MouseClick2("UP3") end)
	UnitPosition:Cheat("Button", " Unit 4 Position", function() MouseClick2("UP4") end)
	UnitPosition:Cheat("Button", " Unit 5 Position", function() MouseClick2("UP5") end)
	UnitPosition:Cheat("Button", " Unit 6 Position", function() MouseClick2("UP6") end)
end
...
function AutoSummon()
	local aaselectbanner = AutoSummonSec:Cheat("Dropdown", "🧙 Select Banner 🧙", function(value) getgenv().SelectedBanner = value end, { options = { "Special", "Standard" } })

	local aaselectbanner = AutoSummonSec:Cheat("Dropdown", "💸 Select Method 💸", function(value) getgenv().SelectedMethod = value end, { options = { "ticket", "gems", "gems10" } })

	AutoSummonSec:Cheat("Checkbox", "Auto Summon", function(bool) getgenv().AutoSummon = bool end)
end
----------------------------------------------
...
if Settings.AutoLoadScript then autoload() end

function autoload2() loadstring(game:HttpGet("https://raw.githubusercontent.com/ArponAG/Scripts/main/AnimeAdventures_v2__Beta.lua"))() end

if Settings.refarmc then autoload2() end
if Settings.setfarm1 then autoload2() end
if Settings.setfarm2 then autoload2() end
if Settings.setfarmIC then autoload2() end
...
local function checkChallenge()
	for i, v in pairs(game.Players.LocalPlayer.PlayerGui:GetChildren()) do
		if v:IsA("SurfaceGui") then
			if v:FindFirstChild("ChallengeCleared") then return v.ChallengeCleared.Visible end
		end
	end
end
		...
		if Settings.AutoChallenge and Settings.AutoFarm and checkReward() == true then
			for i, v in pairs(game:GetService("Workspace")["_CHALLENGES"].Challenges:GetDescendants()) do
				if v.Name == "Owner" and v.Value == nil then
					...
					game:GetService("ReplicatedStorage").endpoints.client_to_server.request_join_lobby:Server(unpack(args))
...
function getBorosPortals()
	...
	for _, item in pairs(get_inventory_items_unique_items()) do
		if item["item_id"] == "portal_boros_g" then table.insert(portals, item) end
	end
	return portals
end
...
function getDemonPortals()
	local portals = {}
	for _, item in pairs(get_inventory_items_unique_items()) do
		if item["item_id"] == "april_portal_item" then table.insert(portals, item) end
	end
	return portals
end
...
function getOPNPortals()
	local portals = {}
	for _, item in pairs(get_inventory_items_unique_items()) do
		if item["item_id"] == "portal_item__dressrosa" then table.insert(portals, item) end
	end
	return portals
end
...
function getZeldrisPortals()
	local portals = {}
	for _, item in pairs(get_inventory_items_unique_items()) do
		if item["item_id"] == "portal_zeldris" then table.insert(portals, item) end
	end
	return portals
end
...
function getMadokaPortals()
	local portals = {}
	for _, item in pairs(get_inventory_items_unique_items()) do
		if item["item_id"] == "portal_item__madoka" then table.insert(portals, item) end
	end
	return portals
end
...
function getberserkPortals()
	local portals = {}
	for _, item in pairs(get_inventory_items_unique_items()) do
		if item["item_id"] == "portal_item__eclipse" then table.insert(portals, item) end
	end
	return portals
end
					...
					if type(v) == "table" then
						if v["session"] then
							for _, item in pairs(v["session"]["inventory"]["inventory_profile_data"]["unique_items"]) do
								if item["item_id"]:match(id) then table.insert(portals, item) end
							end
							return portals
						end
					end
	...
	elseif level == "portal_item__madoka" then
		...
		for i, v in pairs(PortalEvent) do
			if v["_unique_item_data"]["_unique_portal_data"]["portal_depth"] <= Settings.SelectedTier then
				if v["_unique_item_data"]["_unique_portal_data"]["challenge"] ~= Settings.SelectedChallenge and v["_unique_item_data"]["_unique_portal_data"]["challenge"] ~= Settings.SelectedChallenge2 and v["_unique_item_data"]["_unique_portal_data"]["challenge"] ~= Settings.SelectedChallenge3 then
					PortalEventUse = v
					...
					elseif v._stats:FindFirstChild("player") then
						if tostring(v._stats.player.Value) == player then
							...
							elseif v._stats.id.Value == "gojo_evolved" then
								if v._stats.state.Value == "attack" then game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v) end
							--AutoSkill
							...
							elseif v._stats.id.Value ~= "pucci_heaven" then
								if v._stats.state.Value == "attack" then
									if v._stats.active_attack.Value ~= "nil" then game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v) end
								end
							end
						end
					end
					...
					if v._stats.id.Value == "pucci_heaven" then
						if v._stats.state.Value == "attack" then
							...
							if GLD()._gamemode == "infinite" then
								if GetWaveNumber() % 10 == 0 then game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v) end
							-- Check Raid
							elseif GLD()._gamemode == "raid" then
								if GetWaveNumber() == 15 or 20 then game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v) end
							-- Check Story or Infinite Tower
							elseif GLD()._gamemode == "story" or "infinite_tower" then
								if GetWaveNumber() == 15 then game:GetService("ReplicatedStorage").endpoints.client_to_server.use_active_attack:InvokeServer(v) end
							end
						end
					end
				end
...
function autoupgradefunc()
	local success, err = pcall(function() --///
		repeat
			task.wait()
		until game:GetService("Workspace"):WaitForChild("_UNITS")
		for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
			if v:FindFirstChild("_stats") then
				if tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v["_stats"].xp.Value >= 0 then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
			end
		end
	end)
	if err then
...
coroutine.resume(coroutine.create(function()
	while task.wait(2) do
		if Settings.AutoAbilities then
			if game.PlaceId ~= 8304191830 then pcall(function() autoabilityfunc() end) end
		...
		if Settings.AutoUpgrade and not Settings.unitconfig then
			if game.PlaceId ~= 8304191830 then pcall(function() autoupgradefunc() end) end
		...
		if tonumber(v.maxPlayers) > tonumber(v.playing) then
			...
			for _, Existing in pairs(AllIDs) do
				if num ~= 0 then
					...
					if tonumber(actualHour) ~= tonumber(Existing) then local delFile = pcall(function()
...
function GetWaveNumber() return game:GetService("Workspace")["_wave_num"].Value end
		...
		for i, v in pairs(v:GetChildren()) do
			if v.Name == "_stats" then
				...
				if v:FindFirstChild("uuid") and v:FindFirstChild("upgrade") then
					if v.uuid.Value == unitinfo_[2] then table.insert(uu, v.upgrade.Value) end
				end
			end
		end
	end
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		if v:FindFirstChild("_stats") and v:FindFirstChild("_hitbox") then
			if tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v["_stats"].xp.Value >= 0 then
				...
				if v._stats.id.Value == name and v._stats.upgrade.Value <= min then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
			end
		end
	end
end
...
function upgradeunit1(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.upgrade.Value ~= tonumber(Settings.U1_UpgCap) then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function upgradeunit2(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.upgrade.Value ~= tonumber(Settings.U2_UpgCap) then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function upgradeunit3(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.upgrade.Value ~= tonumber(Settings.U3_UpgCap) then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function upgradeunit4(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.upgrade.Value ~= tonumber(Settings.U4_UpgCap) then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function upgradeunit5(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.upgrade.Value ~= tonumber(Settings.U5_UpgCap) then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function upgradeunit6(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.upgrade.Value ~= tonumber(Settings.U6_UpgCap) then game:GetService("ReplicatedStorage").endpoints.client_to_server.upgrade_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function targetpriority(name)
	for i, v in ipairs(Workspace["_UNITS"]:GetChildren()) do
		if v:FindFirstChild("_stats") and v:FindFirstChild("_hitbox") then
			if tostring(v._stats.player.Value) == game.Players.LocalPlayer.Name then
				if v._stats.id.Value == name and v._stats.priority.Value ~= Settings.U1_Priority then game:GetService("ReplicatedStorage").endpoints.client_to_server.cycle_priority:InvokeServer(v) end
			end
		end
	end
end
...
function Priorityunit1(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.priority.Value ~= Settings.U1_Priority then game:GetService("ReplicatedStorage").endpoints.client_to_server.cycle_priority:InvokeServer(v) end
		end
	end
end
...
function Priorityunit2(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.priority.Value ~= Settings.U2_Priority then game:GetService("ReplicatedStorage").endpoints.client_to_server.cycle_priority:InvokeServer(v) end
		end
	end
end
...
function Priorityunit3(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.priority.Value ~= Settings.U3_Priority then game:GetService("ReplicatedStorage").endpoints.client_to_server.cycle_priority:InvokeServer(v) end
		end
	end
end
...
function Priorityunit4(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.priority.Value ~= Settings.U4_Priority then game:GetService("ReplicatedStorage").endpoints.client_to_server.cycle_priority:InvokeServer(v) end
		end
	end
end
...
function Priorityunit5(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.priority.Value ~= Settings.U5_Priority then game:GetService("ReplicatedStorage").endpoints.client_to_server.cycle_priority:InvokeServer(v) end
		end
	end
end
...
function Priorityunit6(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v._stats.id.Value == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name then
			if v._stats.priority.Value ~= Settings.U6_Priority then game:GetService("ReplicatedStorage").endpoints.client_to_server.cycle_priority:InvokeServer(v) end
		end
	end
end
...
function sellunit(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		repeat
			task.wait()
		until v:WaitForChild("_stats")
		if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v._stats:FindFirstChild("upgrade") then game:GetService("ReplicatedStorage").endpoints.client_to_server.sell_unit_ingame:InvokeServer(v) end
	end
end
...
function sellunit1(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v._stats:FindFirstChild("upgrade") then
			if tonumber(Settings.U1_SellWave) <= U1_wave.Value then game:GetService("ReplicatedStorage").endpoints.client_to_server.sell_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function sellunit2(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v._stats:FindFirstChild("upgrade") then
			if tonumber(Settings.U2_SellWave) <= U2_wave.Value then game:GetService("ReplicatedStorage").endpoints.client_to_server.sell_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function sellunit3(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v._stats:FindFirstChild("upgrade") then
			if tonumber(Settings.U3_SellWave) <= U3_wave.Value then game:GetService("ReplicatedStorage").endpoints.client_to_server.sell_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function sellunit4(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v._stats:FindFirstChild("upgrade") then
			if tonumber(Settings.U4_SellWave) <= U4_wave.Value then game:GetService("ReplicatedStorage").endpoints.client_to_server.sell_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function sellunit5(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v._stats:FindFirstChild("upgrade") then
			if tonumber(Settings.U5_SellWave) <= U5_wave.Value then game:GetService("ReplicatedStorage").endpoints.client_to_server.sell_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function sellunit6(name)
	...
	for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
		...
		until v:WaitForChild("_stats")
		if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v._stats:FindFirstChild("upgrade") then
			if tonumber(Settings.U6_SellWave) <= U6_wave.Value then game:GetService("ReplicatedStorage").endpoints.client_to_server.sell_unit_ingame:InvokeServer(v) end
		end
	end
end
...
function reunitcon()
	...
	if Settings.U1_Wave then Settings.U1_Wave = 1 end
	if Settings.U2_Wave then Settings.U2_Wave = 1 end
	if Settings.U3_Wave then Settings.U3_Wave = 1 end
	if Settings.U4_Wave then Settings.U4_Wave = 1 end
	if Settings.U5_Wave then Settings.U5_Wave = 1 end
	if Settings.U6_Wave then Settings.U6_Wave = 1 end
	if Settings.U1_TotalAmmount then Settings.U1_TotalAmmount = 6 end
	if Settings.U2_TotalAmmount then Settings.U2_TotalAmmount = 6 end
	if Settings.U3_TotalAmmount then Settings.U3_TotalAmmount = 6 end
	if Settings.U4_TotalAmmount then Settings.U4_TotalAmmount = 6 end
	if Settings.U5_TotalAmmount then Settings.U5_TotalAmmount = 6 end
	if Settings.U6_TotalAmmount then Settings.U6_TotalAmmount = 6 end
	if Settings.U1_UpgWave then Settings.U1_UpgWave = 1 end
	if Settings.U2_UpgWave then Settings.U2_UpgWave = 1 end
	if Settings.U3_UpgWave then Settings.U3_UpgWave = 1 end
	if Settings.U4_UpgWave then Settings.U4_UpgWave = 1 end
	if Settings.U5_UpgWave then Settings.U5_UpgWave = 1 end
	if Settings.U6_UpgWave then Settings.U6_UpgWave = 1 end
	if Settings.U1_UpgCap then Settings.U1_UpgCap = 99 end
	if Settings.U2_UpgCap then Settings.U2_UpgCap = 99 end
	if Settings.U3_UpgCap then Settings.U3_UpgCap = 99 end
	if Settings.U4_UpgCap then Settings.U4_UpgCap = 99 end
	if Settings.U5_UpgCap then Settings.U5_UpgCap = 99 end
	if Settings.U6_UpgCap then Settings.U6_UpgCap = 99 end
	if Settings.U1_SellWave then Settings.U1_SellWave = 99 end
	if Settings.U2_SellWave then Settings.U2_SellWave = 99 end
	if Settings.U3_SellWave then Settings.U3_SellWave = 99 end
	if Settings.U4_SellWave then Settings.U4_SellWave = 99 end
	if Settings.U5_SellWave then Settings.U5_SellWave = 99 end
	if Settings.U6_SellWave then Settings.U6_SellWave = 99 end
	if Settings.U1_UpgPro then Settings.U1_UpgPro = 1 end
	if Settings.U2_UpgPro then Settings.U2_UpgPro = 1 end
	if Settings.U3_UpgPro then Settings.U3_UpgPro = 1 end
	if Settings.U4_UpgPro then Settings.U4_UpgPro = 1 end
	if Settings.U5_UpgPro then Settings.U5_UpgPro = 1 end
	if Settings.U6_UpgPro then Settings.U6_UpgPro = 1 end
	if Settings.U1_UnPlace then Settings.U1_UnPlace = 1 end
	if Settings.U2_UnPlace then Settings.U2_UnPlace = 1 end
	if Settings.U3_UnPlace then Settings.U3_UnPlace = 1 end
	if Settings.U4_UnPlace then Settings.U4_UnPlace = 1 end
	if Settings.U5_UnPlace then Settings.U5_UnPlace = 1 end
	if Settings.U6_UnPlace then Settings.U6_UnPlace = 1 end
end
if Settings.reunitc then reunitcon() end
...
coroutine.resume(coroutine.create(function()
	while task.wait(0.5) do
		if game.PlaceId ~= 8304191830 and Settings.AutoFarm and Settings.unitconfig and not getgenv().disableatuofarm then
			...
			repeat
				task.wait()
			until game:GetService("Workspace"):WaitForChild("_map")
			if game.Workspace._map:FindFirstChild("namek mushroom model") then
...
function hidename()
	task.spawn(function() -- Hides name for yters (not sure if its Fe)
		while task.wait() do
			pcall(function()
				if game.Players.LocalPlayer.Character.Head:FindFirstChild("_overhead") then game.Players.LocalPlayer.Character.Head:FindFirstChild("_overhead"):Destroy() end
			end)
		end
	end)
end
if Settings.hidenamep then hidename() end
...
if Settings.autoDailyquest then autoDailyquest() end
	...
	if game.Workspace:WaitForChild("_UNITS") then
		for i, v in ipairs(game:GetService("Workspace")["_UNITS"]:GetChildren()) do
			repeat
				task.wait()
			until v:WaitForChild("_stats")
			if v.Name == name and tostring(v["_stats"].player.Value) == game.Players.LocalPlayer.Name and v.Name:FindFirstChild("_hitbox") then v:Destroy() end
		end
	end
end
...
function Reedemcode()
	codes = { "TWOMILLION", "subtomaokuma", "CHALLENGEFIX", "GINYUFIX", "RELEASE", "SubToKelvingts", "SubToBlamspot", "KingLuffy", "TOADBOIGAMING", "noclypso", "FictioNTheFirst", "GOLDENSHUTDOWN", "GOLDEN", "SINS2", "subtosnowrbx", "Cxrsed", "subtomaokuma", "VIGILANTE", "HAPPYEASTER", "ENTERTAINMENT", "DRESSROSA", "BILLION", "MADOKA", "AINCRAD", "ANNIVERSARY" }
	for _, v in pairs(codes) do
		pcall(function() game:GetService("ReplicatedStorage").endpoints["client_to_server"]["redeem_code"]:InvokeServer(v)() end)
	end
end

if Settings.redeemc then Reedemcode() end
...
if game.PlaceId == 8304191830 then
	repeat
		task.wait(0.5)
	until Workspace:WaitForChild(game.Players.LocalPlayer.Name)
	checkInterNet()
elseif game.PlaceId ~= 8304191830 then
	repeat
		task.wait(0.5)
	until Workspace:WaitForChild("_terrain")
	checkInterNet()
end
